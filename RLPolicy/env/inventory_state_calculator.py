...
class StateCalculator:
    ...
    def _add_uncontrollable_part_feature(self, state, _facility):
        ...
        state['uncontrollable_part_state_cnn'] = np.zeros((7, 21))
        if isinstance(_facility, OuterSKUStoreUnit):
            data_info = _facility.seller.info
            date_info_hist = _facility.seller.date_info_hist
            ...
            demand_future = [x / data_info['demand_mean'] for x in data_info['demand_pred']]
            data_out = demand_future[0:3].copy()
            data_out.append(np.mean(demand_future[:3]))
            data_out.append(np.mean(demand_future[:7]))
            data_out.append(np.mean(demand_future))

            demand_history = [x / data_info['demand_mean'] for x in data_info['demand_hist']]
            data_in = demand_history.copy()
