...
all_sale_sampler = [online_sale_sampler(f"data/OnlineRetail/store{i+1}_new.csv") for i in range(2)]
# all_sale_sampler = [online_sale_sampler(f"data/OnlineRetail/store{i+1}.csv") for i in range(2)]
        ...
        for i in range(n_stores):
            ...
            for _, sku_info in enumerate(sku_info_list):
                ...
                if Utils.get_demand_sampler() == "DYNAMIC_GAMMA":
                    ...
                    sale_sampler = all_sale_sampler[i]
                    sku = OuterSKUStoreUnit(r, retail_sku_config, default_economy_config(order_cost=r.facility_info['order_cost']), sale_sampler )
                sku.idx_in_config = sku_info['sku_name']
...
class InventoryManageEnv(MultiAgentEnv):
    def __init__(self, env_config):
        ...
        self.done_step = self.env_config['episod_duration']
    ...
    def reset(self, eval=False, eval_on_trainingset=False):
        # print("Rst!")
        ...
        self.done_step = global_config.training_length

        if global_config.oracle or ( eval and (not eval_on_trainingset) ):
            # self.world = WorldBuilder.create(80, 16)
            self.set_retailer_step(self.env_config['episod_duration'])
            self.done_step = self.env_config['evaluation_len']
    ...
    def step(self, action_dict):
        ...
        all_done = (self.world.time_step >= self.done_step)
