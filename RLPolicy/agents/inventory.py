...
class FacilityCell(Cell, Agent):
    ...
    def reset(self):
        ...
        for sku in self.sku_in_stock:
            sku.reset()
            # init_stock = np.random.randint(sku.sku_info.get('init_stock', 100))  #only for retail
            init_stock = sku.sku_info.get('init_stock', 100)
...
class RetailerCell(FacilityCell):
    ...
    def reset(self):
        ...
        for sku in self.sku_in_stock:
            sku.reset()
            # print(self, sku, sku.get_sale_mean())
            # print("")
            init_stock = np.random.randint(5 * sku.get_sale_mean())
            self.storage.try_add_units({sku.sku_info['sku_name']: init_stock})
